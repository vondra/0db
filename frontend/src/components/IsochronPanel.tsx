  ...
  return (
    ...
    ">
      <div className="flex items-center gap-2 text-sm">
        ...
        <Button type="button" variant={walk ? 'default' : 'outline'} size="xs" onClick={() => setWalk(!walk)} className={walk ? '' : 'text-muted-foreground'}>Walk</Button>
        <Button type="button" variant={car ? 'default' : 'outline'} size="xs" onClick={() => setCar(!car)} className={car ? '' : 'text-muted-foreground'}>Car</Button>
        <Button size="xs" onClick={handleGo} disabled={!location || (!walk && !car)} className="ml-auto">Show area</Button>
      </div>
