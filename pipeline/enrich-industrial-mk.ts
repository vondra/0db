...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/mk`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// North Macedonia bbox [minLat, minLon, maxLat, maxLon]
const MK_BBOX: [number, number, number, number] = [40.85, 20.4, 42.4, 23.05]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Serbia N — above lat 42.35 west of 21.8
  [42.35, 20.4, 42.4, 21.8],
  // Kosovo NW — above lat 42.0 west of 21.0
  [42.0, 20.4, 42.4, 21.0],
  // Albania W — west of 20.55
  [40.85, 20.4, 42.4, 20.55],
  // Greece S — below lat 41.0
  [40.85, 20.4, 41.0, 23.05],
  // Bulgaria E — east of 22.9
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, MK_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'MK plant').toString(),
...
async function main() {
  console.log(`=== MK Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in MK: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, MK_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  MK-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM MK (${best.fuel})` }
          matched++
        }
      }
    } catch {}
