 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-roads-pe.ts
 */
...
async function main() {
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, PE_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'roads.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!inBbox(midLat, midLon, PE_BBOX)) continue
