...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/by`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Belarus bbox [minLat, minLon, maxLat, maxLon]
const BY_BBOX: [number, number, number, number] = [51.2, 23.1, 56.2, 32.8]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Russia E — east of 32.5
  [51.2, 32.5, 56.2, 32.8],
  // Ukraine S — below lat 51.5
  [51.2, 23.1, 51.5, 32.8],
  // Poland W — west of 23.5
  [51.2, 23.1, 56.2, 23.5],
  // Lithuania NW — above lat 54.5 west of 26
  [54.5, 23.1, 56.2, 26.0],
  // Latvia N — above lat 55.8
  [55.8, 23.1, 56.2, 32.8],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, BY_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'BY plant').toString(),
...
async function main() {
  console.log(`=== BY Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in BY: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, BY_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  BY-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM BY (${best.fuel})` }
          matched++
        }
      }
    } catch {}
