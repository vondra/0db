...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/li`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Liechtenstein bbox: [minLat, minLon, maxLat, maxLon]
const LI_BBOX: [number, number, number, number] = [47.05, 9.47, 47.27, 9.64]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, LI_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'LI plant').toString(),
...
async function main() {
  console.log(`=== LI Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)
  console.log(`  Note: GEM reports 0 operating plants for Liechtenstein (imports all electricity).\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in LI: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, LI_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  LI-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM LI (${best.fuel})` }
          matched++
        }
      }
    } catch {}
