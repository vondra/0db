...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/sn`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Senegal bbox
const SN_BBOX: [number, number, number, number] = [12.3, -17.6, 16.7, -11.3]
...
const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  ...
  // Guinea (SE)
  [12.3, -13.7, 12.7, -11.3],
  // Gambia (entirely enclosed — special case, we KEEP this inside SN bbox
  // because Gambia's road network is internally connected via SN; skip exclude)
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, SN_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'SN plant').toString(),
...
async function main() {
  console.log(`=== SN Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in SN: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, SN_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  SN-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM SN (${best.fuel})` }
          matched++
        }
      }
    } catch {}
