...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/sm`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// San Marino bbox: [minLat, minLon, maxLat, maxLon]
const SM_BBOX: [number, number, number, number] = [43.89, 12.40, 43.99, 12.52]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, SM_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'SM plant').toString(),
...
async function main() {
  console.log(`=== SM Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)
  console.log(`  Note: GEM reports 0 operating plants for San Marino. SM bbox within IT enriched area.\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in SM: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, SM_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  SM-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM SM (${best.fuel})` }
          matched++
        }
      }
    } catch {}
