/**
 * Enrich SE railways.arrow with GTFS Sverige 2 unified train/tram/metro frequencies.
 *
 ...
 * Flixtrain. Single 58 MB zip, updated daily.
 *
 * Usage:
...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  allStopCounts = allStopCounts.filter((sc) => inSe(sc.lat, sc.lon))
  ...
  // Scan ALL Swedish hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits a mainline count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, SE_BBOX, 'railways.arrow')
  console.log(`  SE hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → national rail stops; tram/light_rail
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside SE (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inSe, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inSe, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
...
async function main() {
  console.log(`=== SE Railway Enrichment — GTFS Sverige 2 (${YEAR}) ===\n`)
