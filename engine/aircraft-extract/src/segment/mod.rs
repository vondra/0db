//! Per-sample airborne segmentation.
//!
//! For each consecutive ADS-B sample-pair `(points[i-1], points[i])` we
//! emit one [`FlightSegment`]. No aggregation: collapsing many samples
//! into one straight chord loses curving-flight geometry that the kernel
//! then mis-locates. Doc 29 §3.6.4 calls for sub-segmentation at
//! significant trajectory changes; per-sample IS the finest such sampling
//! and ADS-B cadence (1-15 s typical) gives natural 1-4 km segments at
//! cruise speed — short enough that `Λ(β)` and `ΔF` are well-defined
//! within one segment.
//!
//! Per flight, after Stage 1 has phase-classified each point:
//!   * Segment phase: if either endpoint is Airborne → Airborne (covers
//!     takeoff Ground→Airborne, flare Airborne→Ground, and the descent
//!     transition Cruise→Airborne without forcing Stage 2B departure-
//!     NPD on a descending leg). Otherwise inherit the later point's
//!     phase: both Ground → Ground, both Cruise → Cruise,
//!     Airborne→Cruise climb → Cruise.
//!   * Phase-aware time-gap policy — sample-pair dropped when `dt`
//!     exceeds [`GAP_S_CRUISE`] / [`GAP_S_AIRBORNE`] / [`GAP_S_GROUND`].
//!     Budget keyed to the more permissive endpoint phase so a cruise
//!     dropout reappearing as airborne still gets the 1 h budget.
//!     Beyond that, linear interpolation through the gap would route
//!     contributions through unknown geometry; better to drop than to
//!     fabricate.
//!   * `is_departure` per sample-pair from [`classify_is_departure_per_sample`]
//!     — smoothed local ROCD with Doc 29 §A.3.2 thresholds for airborne
//!     pairs, smoothed local speed acceleration above
//!     [`GROUND_DEPARTURE_ACCEL_KT_PER_MIN`] for on-ground pairs.
//!   * Ground-endpoint pairs (one endpoint has `airborne_alt_ft = None`)
//!     inherit the airborne endpoint's altitude for both endpoints so
//!     elevated-airport takeoffs don't fail the downstream
//!     terrain-vs-altitude airborne validation with a fictional 0 m MSL.
//!   * Receiver-independent validity (length, AGL floor, speed) via
//!     [`crate::filters::segment_is_keepable`].
//!
//! Output is a vector of [`FlightSegment`] rows ready for
//! `segments/<day>.arrow`.
...
/// A flight leg ends when consecutive [`TracePoint::on_ground_raw`]
...
pub fn build_segments(
    ...
    for i in 1..points.len() {
        ...
        let phase = segment_phase(phases[i - 1], phases[i]);
...
/// Segment phase for a sample-pair. Airborne wins over both Ground and
/// Cruise: Ground↔Airborne transitions keep takeoff/flare audible (vs
/// dropping into ground-roll); Cruise→Airborne descent stays Airborne
/// (vs Stage 2B forcing Departure NPD on a descending leg). Same-phase
/// pairs and the Airborne→Cruise climb fall through to the later point.
#[inline]
fn segment_phase(prev: Phase, curr: Phase) -> Phase {
    if prev == Phase::Airborne || curr == Phase::Airborne {
        Phase::Airborne
    } else {
        curr
    }
