...
//! the property test below). A mega-hub R4's gate drops from ~30 min to
...
fn cell_of(lat: f64, lon: f64) -> (i32, i32) {
    ...
    let cx = (lon / (CELL_M / band_lon_scale(cy))).floor() as i32;
    (cy, cx)
}
    ...
    pub fn build(areas: &'a [AirportArea]) -> Self {
        ...
        for (i, a) in areas.iter().enumerate() {
            ...
            for cy in cy_min..=cy_max {
                ...
                let lon_cell_deg = CELL_M / lon_scale;
                let dlon = radius / lon_scale;
                let cx_min = ((a.centroid_lon - dlon) / lon_cell_deg).floor() as i32 - 1;
                let cx_max = ((a.centroid_lon + dlon) / lon_cell_deg).floor() as i32 + 1;
                for cx in cx_min..=cx_max {
                    cells.entry((cy, cx)).or_default().push((i as u32, radius));
                }
            }
        }
        Self { cells, areas }
