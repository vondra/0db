 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-industrial-eg.ts
 */
...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/eg`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Egypt bbox (Sinai + Western Desert + Eastern Desert)
...
const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  ...
  // Sudan (S)
  [22.0, 24.7, 22.1, 36.9],
  // Israel + Gaza (NE Sinai — Rafah)
  [29.5, 34.25, 31.7, 34.9],
  // Saudi Arabia (Gulf of Aqaba E)
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, EG_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'EG plant').toString(),
...
async function main() {
  console.log(`=== EG Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in EG: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, EG_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  EG-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM EG (${best.fuel})` }
          matched++
        }
      }
    } catch {}
