...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/mm`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Myanmar bbox [minLat, minLon, maxLat, maxLon]
const MM_BBOX: [number, number, number, number] = [9.5, 92.0, 28.5, 101.2]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // India NW — west of 93.5 above lat 22
  [22.0, 92.0, 28.5, 93.5],
  // Bangladesh W — west of 92.8, lat 20–22
  [20.0, 92.0, 22.0, 92.8],
  // China NE — east of 97 above lat 24
  [24.0, 97.0, 28.5, 101.2],
  // Laos E — east of 100 below lat 22
  [9.5, 100.0, 22.0, 101.2],
  // Thailand SE — east of 98 below lat 18
  [9.5, 98.0, 18.0, 101.2],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, MM_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'MM plant').toString(),
...
async function main() {
  console.log(`=== MM Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in MM: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, MM_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  MM-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM MM (${best.fuel})` }
          matched++
        }
      }
    } catch {}
