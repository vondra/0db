 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-industrial-mc.ts
 */
...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/mc`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Monaco bbox: [minLat, minLon, maxLat, maxLon]
const MC_BBOX: [number, number, number, number] = [43.72, 7.40, 43.76, 7.44]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, MC_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'MC plant').toString(),
...
async function main() {
  console.log(`=== MC Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)
  console.log(`  Note: GEM reports 0 operating plants for Monaco. MC bbox within FR enriched area.\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in MC: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, MC_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  MC-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM MC (${best.fuel})` }
          matched++
        }
      }
    } catch {}
