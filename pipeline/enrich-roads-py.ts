 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-roads-py.ts
 */
...
const MY_DATASET_ID = DATASETS_BY_KEY.get('py-national-roads')!.id
...
async function main() {
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, PY_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'roads.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  PY-bbox hexes with roads.arrow: ${hexDirs.length}`)

  let totalRoads = 0, excluded = 0, alreadyEnriched = 0
  let matchedMopc = 0, matchedClass = 0
  let hexesUpdated = 0
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!inBbox(midLat, midLon, PY_BBOX)) continue
