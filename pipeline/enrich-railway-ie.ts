/**
 * Enrich IE railways.arrow with NTA unified GTFS train + tram frequencies.
 *
...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  // so the raw stop list may contain foreign stations (NTA GTFS includes the
  // cross-border Enterprise/Translink legs into Northern Ireland) — joining those
  // would stamp a neighbour's track under this feed's id, and the same-rank
  // higher-id tiebreak can beat the neighbour's own national source (mechanism:
  // the PL feed stamped 11,856 km of CZ track, 7fac2349). A national feed only
  // speaks for its own country's network: foreign stops are dropped BEFORE any
  // grid is built.
  const inIe = makeCountryGate('IE')
  const rawCount = allStopCounts.length
  allStopCounts = allStopCounts.filter((sc) => inIe(sc.lat, sc.lon))
  ...
  // Scan ALL Irish hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits Irish Rail's count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, IE_BBOX, 'railways.arrow')
  console.log(`  IE hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → Irish Rail stops; tram/light_rail
        // (rail_type 1/2) → Luas stops. Cross-family matches can't happen.
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside IE (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inIe, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inIe, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
...
async function main() {
  console.log(`=== IE Railway Enrichment — NTA unified GTFS (${YEAR}) ===\n`)
