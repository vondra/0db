...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/is`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Iceland bbox — island, no neighbor excludes needed
const IS_BBOX: [number, number, number, number] = [63.3, -24.6, 66.6, -13.4]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, IS_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'IS plant').toString(),
...
async function main() {
  console.log(`=== IS Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in IS: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, IS_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  IS-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM IS (${best.fuel})` }
          matched++
        }
      }
    } catch {}
