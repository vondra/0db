...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/kh`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Cambodia bbox [minLat, minLon, maxLat, maxLon]
const KH_BBOX: [number, number, number, number] = [10.4, 102.3, 14.7, 107.7]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Thailand W — west of 103.0 above lat 12
  [12.0, 102.3, 14.7, 103.0],
  // Laos N — above lat 14.3
  [14.3, 102.3, 14.7, 107.7],
  // Vietnam E — east of 107.0 below lat 14.3
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, KH_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'KH plant').toString(),
...
async function main() {
  console.log(`=== KH Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in KH: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, KH_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  KH-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM KH (${best.fuel})` }
          matched++
        }
      }
    } catch {}
