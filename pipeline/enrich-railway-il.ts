...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  allStopCounts = allStopCounts.filter((sc) => inIl(sc.lat, sc.lon))
  ...
  // Scan ALL Israeli hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits Israel Railways' count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, IL_BBOX, 'railways.arrow')
  console.log(`  IL hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → Israel Railways rail stops; tram/light_rail
        // (rail_type 1/2) → Tel Aviv/Jerusalem LRT stops. Cross-family matches can't happen.
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside IL (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inIl, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inIl, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
