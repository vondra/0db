...
//! * CPU kernels take the popup's vector form — [`Barrier`] midpoints whose
//!   `dist_m` early-break contract is documented on the type. The per-tile
//!   slice is prepared by [`BarrierData::for_tile`] (filter + conservative
//!   lower-bound distance + ascending sort), mirroring source-reader
...
impl BarrierData {
    ...
    ///
    /// `reach_m` is the build's max source reach (the shared halo radius):
    /// a barrier only matters on some source→receiver path, every point of
    /// which lies within `reach_m` of a receiver pixel, so midpoints beyond
    /// `reach_m + tile_half_diagonal` of the tile centre can never project
    /// onto any such path (same horizon the popup uses with its exact 10 km
    /// per-receiver load).
    ///
    ...
    pub fn for_tile(&self, bbox: &TileBbox, reach_m: f64) -> Vec<Barrier> {
        ...
        let mut out: Vec<Barrier> = self
            ...
            .filter_map(|s| {
                ...
                if d_centre > reach_m + half_diag + DIST_SLACK_M {
                    return None;
                }
                Some(Barrier {
                    osm_id: s.osm_id,
    ...
    fn for_tile_dist_is_conservative_and_sorted() {
        ...
        for b in &out {
            for (lat, lon) in [
                ...
                let true_d = flat_dist(lat, lon, b.lat, b.lon);
                assert!(
