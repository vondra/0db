...
/// Where between samples (i-1, i) does the linearly interpolated AGL
/// cross [`PHASE_BOUNDARY_AGL_M`]? Returns `f ∈ (0, 1)` along the
/// segment for use as a phase-aware break point. Returns `None` when
/// the boundary is not crossed.
pub fn boundary_fraction(prev_agl_m: f32, curr_agl_m: f32) -> Option<f32> {
    let lo = prev_agl_m.min(curr_agl_m);
    let hi = prev_agl_m.max(curr_agl_m);
    if lo >= PHASE_BOUNDARY_AGL_M || hi <= PHASE_BOUNDARY_AGL_M {
        return None;
    }
    let span = curr_agl_m - prev_agl_m;
    if span.abs() < f32::EPSILON {
        return None;
    }
    let f = (PHASE_BOUNDARY_AGL_M - prev_agl_m) / span;
    if f > 0.0 && f < 1.0 {
        Some(f)
    } else {
        None
...
mod tests {
    ...
    #[test]
    fn boundary_fraction_finds_crossing() {
        let f = boundary_fraction(7000.0, 9000.0).unwrap();
        assert!((f - 0.31).abs() < 1e-3);
        let f2 = boundary_fraction(9000.0, 7000.0).unwrap();
        assert!((f2 - 0.69).abs() < 1e-3);
    }

    #[test]
    fn boundary_fraction_none_when_not_crossed() {
        assert!(boundary_fraction(8000.0, 9000.0).is_none());
        assert!(boundary_fraction(5000.0, 6000.0).is_none());
    }
