/**
 * Enrich AU railways.arrow with state-level Australian GTFS feeds.
 *
 * Sources:
 *   NSW Sydney unified GTFS — Sydney Trains + NSW TrainLink + Sydney Metro + light rail (310 MB)
 *   Adelaide Metro SA — trains + bus (12.8 MB)
 *   Transperth WA — trains + bus + ferry (29.8 MB)
 *
 * Victoria (PTV) and Queensland (Translink) are already covered via continental
...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  allStopCounts = allStopCounts.filter((sc) => inAu(sc.lat, sc.lon))
  ...
  // Scan ALL Australian hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits a heavy-rail count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, AU_BBOX, 'railways.arrow')
  console.log(`  AU hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → suburban/intercity rail stops; tram/light_rail
        // (rail_type 1/2) → Melbourne/Adelaide tram + metro stops. Cross-family matches can't happen.
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside AU (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inAu, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inAu, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
...
async function main() {
  console.log(`=== AU Railway Enrichment — Multi-feed GTFS (${YEAR}) ===\n`)
  ...
  // snapshot — all three feeds loaded non-empty THIS run (or a v2 cache that
