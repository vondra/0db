...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/md`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Moldova bbox [minLat, minLon, maxLat, maxLon]
const MD_BBOX: [number, number, number, number] = [45.4, 26.6, 48.5, 30.2]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Ukraine E — east of 30.1
  [45.4, 30.1, 48.5, 30.2],
  // Romania W — west of 26.65
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, MD_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'MD plant').toString(),
...
async function main() {
  console.log(`=== MD Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in MD: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, MD_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  MD-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM MD (${best.fuel})` }
          matched++
        }
      }
    } catch {}
