...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/ba`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Bosnia and Herzegovina bbox [minLat, minLon, maxLat, maxLon]
const BA_BBOX: [number, number, number, number] = [42.55, 15.7, 45.3, 19.65]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Croatia N — above lat 45.1 west of 17.5
  [45.1, 15.7, 45.3, 17.5],
  // Croatia W — west of 16.3
  [42.55, 15.7, 45.3, 16.3],
  // Serbia E — east of 19.5 above lat 43.5
  [43.5, 19.5, 45.3, 19.65],
  // Montenegro SE — below lat 42.8 east of 18.3
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, BA_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'BA plant').toString(),
...
async function main() {
  console.log(`=== BA Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in BA: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, BA_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  BA-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM BA (${best.fuel})` }
          matched++
        }
      }
    } catch {}
