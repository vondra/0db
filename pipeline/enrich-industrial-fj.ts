 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-industrial-fj.ts
 */
...
async function main() {
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inFiji(lat, lon) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  FJ-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM FJ (${best.fuel})` }
          matched++
        }
      }
    } catch {}
