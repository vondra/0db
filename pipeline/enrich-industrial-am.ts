...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/am`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Armenia bbox [minLat, minLon, maxLat, maxLon]
const AM_BBOX: [number, number, number, number] = [38.8, 43.4, 41.35, 46.65]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Turkey W — west of 43.6
  [38.8, 43.4, 41.35, 43.6],
  // Georgia N — above lat 41.2
  [41.2, 43.4, 41.35, 46.65],
  // Azerbaijan E — east of 46.0
  [38.8, 46.0, 41.35, 46.65],
  // Iran S — below lat 39.0
  [38.8, 43.4, 39.0, 46.65],
  // Azerbaijan Nakhchivan — below lat 39.6, west of 45.0, above lat 38.8
  [38.8, 43.4, 39.6, 45.0],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, AM_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'AM plant').toString(),
...
async function main() {
  console.log(`=== AM Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in AM: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, AM_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  AM-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM AM (${best.fuel})` }
          matched++
        }
      }
    } catch {}
