...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/iq`)
...
const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Turkey N — above lat 37.2 west of 45
  [37.2, 38.7, 37.4, 45.0],
  // Iran E — east of 46.5 above lat 32
  [32.0, 46.5, 37.4, 48.6],
  // Iran E — east of 48 below lat 32
  [29.0, 48.0, 32.0, 48.6],
  // Kuwait SE — below lat 29.5 east of 46.5
  [29.0, 46.5, 29.5, 48.6],
  // Saudi Arabia S — below lat 29.4
  [29.0, 38.7, 29.4, 48.6],
  // Syria W — west of 40.5 above lat 33
  [33.0, 38.7, 37.4, 40.5],
  // Jordan SW — west of 39.5 below lat 33
  [29.0, 38.7, 33.0, 39.5],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, IQ_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'IQ plant').toString(),
...
async function main() {
  console.log(`=== IQ Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in IQ: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, IQ_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  IQ-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM IQ (${best.fuel})` }
          matched++
        }
      }
    } catch {}
