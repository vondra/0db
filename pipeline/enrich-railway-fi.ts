/**
 * Enrich FI railways.arrow with multi-feed Finnish GTFS train + tram + metro frequencies.
 *
 * Sources:
 *   rata.digitraffic.fi/api/v1/trains/gtfs-passenger.zip — Fintraffic VR national rail
 *   infopalvelut.storage.hsldev.com/gtfs/hsl.zip — HSL Helsinki commuter + metro + tram
 *   data.itsfactory.fi/journeys/files/gtfs/latest/gtfs_tampere.zip — Tampere tram + bus
 *   data.foli.fi/gtfs/gtfs.zip — Turku/Föli (no tram, but kept for completeness)
 *
 * Usage:
...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  allStopCounts = allStopCounts.filter((sc) => inFi(sc.lat, sc.lon))
  ...
  // Scan ALL Finnish hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits VR's count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, FI_BBOX, 'railways.arrow')
  console.log(`  FI hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → VR rail stops; tram/light_rail
        // (rail_type 1/2) → HSL/Tampere tram stops. Cross-family matches can't happen.
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside FI (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inFi, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inFi, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
...
async function main() {
  console.log(`=== FI Railway Enrichment — Multi-feed GTFS (${YEAR}) ===\n`)
