...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/dj`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Djibouti bbox: [minLat, minLon, maxLat, maxLon]
const DJ_BBOX: [number, number, number, number] = [10.9, 41.7, 12.8, 43.5]
...
/**
 * Returns true if the point is clearly outside Djibouti.
 */
function isExcluded(lat: number, lon: number): boolean {
  if (lat > 12.7) return true          // Eritrea N
  if (lat < 11.0) return true          // Somalia / Ethiopia S
  if (lon < 42.0) return true          // Ethiopia W
  return false
}
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, DJ_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'DJ plant').toString(),
...
async function main() {
  console.log(`=== DJ Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in DJ: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, DJ_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  DJ-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM DJ (${best.fuel})` }
          matched++
        }
      }
    } catch {}
