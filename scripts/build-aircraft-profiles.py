...
def emit_rust(
    ...
    lines.append("/// business jet → CRJ9, Cirrus/Diamond/RV → C172, helicopters → EC35).")
    ...
    lines.append('    if matches!(b, [b\'C\', b\'L\', _, _] | [b\'G\', b\'L\', _, _] | [b\'H\', b\'2\', b\'5\', _]) {')
    lines.append('        return profile_idx("CRJ9");')
    lines.append("    }")
    ...
    lines.append('        return profile_idx("EC35");')
    lines.append("    }")
    lines.append("    // Sikorsky H-60 / S-70 / S-76 / S-92, Robinson R22/44/66")
    lines.append("    // (3-char ICAO codes — match by length and prefix)")
    lines.append('    if matches!(b, b"H60" | b"R22" | b"R44" | b"R66" | b"S70" | b"S76" | b"S92") {')
    lines.append('        return profile_idx("EC35");')
    lines.append("    }")
