//! Host/cgroup RAM probing for the `run-all` Stage 0/1 day-concurrency
//! cap. Sits beside the `aircraft-extract` bin's dispatcher; consumed by
//! `cli_runners::run_all` to size `days.chunks(max_concurrent)` to the
//! smaller of host RAM and this process's cgroup limit.
