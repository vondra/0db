    ...
    fn metadata_populated_with_arr_dep_split_and_profile_mix() {
        ...
        let Some(SourceMetadata::Aircraft(md)) = &out[0].metadata else {
            panic!("expected Some(SourceMetadata::Aircraft), got {:?}", out[0].metadata);
        };
    ...
    fn far_receiver_pruned() {
        ...
        let receiver = Receiver { lat: 0.0, lon: 14.255, elevation_m: 0.0, height_m: 4.0 };
        let out = run_flat(&receiver, &[row], 1);
        assert!(out.is_empty());
    }
    ...
    fn rows_aggregate_by_airport_key() {
        ...
        row2.end_lon = 14.257;
        let receiver = Receiver { lat: 50.105, lon: 14.260, elevation_m: 0.0, height_m: 4.0 };
    ...
    fn geometry_dedups_replicated_microseg_rows() {
        ...
        let coords = geom.get("coordinates").and_then(|v| v.as_array()).expect("coords array");
    ...
    impl RasterSampler for MockHilly {
        ...
        fn ground_g(&self, _: f64, _: f64) -> f64 { self.g }
    ...
    fn pe_receiver() -> Receiver {
        Receiver { lat: PE_RCV_LAT, lon: PE_RCV_LON, elevation_m: 0.0, height_m: 4.0 }
    }
