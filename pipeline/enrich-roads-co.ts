...
const MY_DATASET_ID = DATASETS_BY_KEY.get('co-national-roads')!.id
...
async function main() {
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, CO_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'roads.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  CO-bbox hexes with roads.arrow: ${hexDirs.length}`)

  let totalRoads = 0, excluded = 0, alreadyEnriched = 0
  let matchedTpda = 0, matchedRvn = 0, matchedClass = 0
  let hexesUpdated = 0
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!inBbox(midLat, midLon, CO_BBOX)) continue
