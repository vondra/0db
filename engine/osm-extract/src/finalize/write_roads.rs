//! `roads.arrow` writer: one row per road microsegment with geometry +
//! classification + provenance columns. See `finalize_bucket` for the dispatch.
...
pub(super) fn write_roads(rows: &[Vec<String>], path: &Path) -> Result<()> {
    ...
    let schema = Schema::new(vec![
        ...
        // See pipeline/lib/sources.ts for the registry; provenance kind
        // (measured/heuristic/baseline/none) derives from this id via
        // noise_compute::sources::provenance_of().
