    ...
    /// Hostivice false-positive class regression: a cluster inside
    /// an aerodrome's polygon buffer but FAR from any real OSM
    /// aeroway line is NOT folded into the airport's key — it's
    /// relabeled as `auto-<R11>` so the popup shows it under the
    /// neutral "Auto airfield X,Y" label instead of the misleading
    /// real airport name. The cluster stays visible (no silent
    /// drop) and a future metadata column can flag its
    /// in-airport-buffer status for visual triage.
    #[test]
    ...
    /// 30+ min per hub R4. With the polygon gate, both endpoints of a
    ...
    /// input at hub R4s without aiding discovery.
    #[test]
    ...
    /// Materialise a per-R4 ground shard the way `shuffle_per_r4`
    /// would for tests. Returns `(tmp_root, segments_by_r4_dir,
    /// h3r4_dir)` so callers can pass `h3r4_dir` through and inspect
    /// the same dir later.
    fn materialise_per_r4(
        ...
        // Group segs by midpoint R4.
        let mut by_r4_map: std::collections::HashMap<u64, Vec<FlightSegment>> =
        ...
        for seg in segs {
            ...
            by_r4_map.entry(r4).or_default().push(seg.clone());
        }
        for (r4, segs) in by_r4_map {
            let dir = by_r4.join(r4_hex_str(r4));
    ...
    #[test]
    fn empty_input_returns_zero_populated_r4s() {
        let tmp = tempfile::tempdir().unwrap();
        let by_r4 = tmp.path().join("segments_by_r4");
        std::fs::create_dir_all(&by_r4).unwrap();
        let n =
            run_stage_airport_discover(&by_r4, &AerodromeIndex::build(&[]), &[], tmp.path(), None)
                .unwrap();
        assert_eq!(n, 0);
    }
    ...
    fn only_r4_dir(root: &Path) -> std::path::PathBuf {
        // `root` may not exist if no R4 was emitted; treat as empty.
        let Ok(read) = std::fs::read_dir(root) else {
            panic!(
                "expected exactly one R4 dir, dir missing: {}",
                root.display()
            );
        };
        let dirs: Vec<_> = read
        ...
        assert_eq!(
            dirs.len(),
            1,
            "expected exactly one R4 dir, got {}",
            dirs.len()
        );
        dirs[0].path()
    }
    ...
    fn end_to_end_emits_synth_arrows_for_an_unmapped_strip() {
        let tmp = tempfile::tempdir().unwrap();
        let (by_r4, h3r4) = materialise_per_r4(&tmp, &unmapped_strip_segments());
        let n = run_stage_airport_discover(&by_r4, &AerodromeIndex::build(&[]), &[], &h3r4, None)
            .unwrap();
        assert_eq!(n, 1, "expected exactly one R4 to receive synth rows");
        let dir = only_r4_dir(&h3r4);
        let lines = read_synth_airport_lines(&dir.join(SYNTH_LINES_FILE)).unwrap();
        let areas = read_synth_airport_areas(&dir.join(SYNTH_AREAS_FILE)).unwrap();
    ...
    /// R4 drops out of the current run's ground-segment set entirely.
    /// The on-disk scan must rediscover it and clear the stale sidecars
    /// — otherwise Stage 2C consumes zombie airport areas.
    #[test]
    fn rerun_clears_stale_synth_files_when_r4_drops_out() {
        let tmp = tempfile::tempdir().unwrap();
        let (by_r4, h3r4) = materialise_per_r4(&tmp, &unmapped_strip_segments());
        run_stage_airport_discover(&by_r4, &AerodromeIndex::build(&[]), &[], &h3r4, None).unwrap();
        let dir = only_r4_dir(&h3r4);
        ...
        // Empty second run — wipe the segments_by_r4 dir so the R4 has
        // no shard, exercise the stale-sidecar path.
        std::fs::remove_dir_all(&by_r4).unwrap();
        std::fs::create_dir_all(&by_r4).unwrap();
        run_stage_airport_discover(&by_r4, &AerodromeIndex::build(&[]), &[], &h3r4, None).unwrap();
        assert_sidecars_empty(&dir);
    }

    /// R4 stays in scope (one distant non-clustering segment) but no
    ...
    fn rerun_clears_stale_synth_files() {
        ...
        // Second run with one distant segment in the SAME R4 (so the
        // shard isn't empty but doesn't cluster).
        let dlon = (1000.0 / 111_320.0) as f32 / ANCHOR_LAT.to_radians().cos();
        let kept = ground_segment_at(ANCHOR_LAT, ANCHOR_LON, ANCHOR_LAT, ANCHOR_LON + dlon / 60.0);
        let (by_r4_b, _) = materialise_per_r4(&tmp, std::slice::from_ref(&kept));
        run_stage_airport_discover(&by_r4_b, &AerodromeIndex::build(&[]), &[], &h3r4, None)
            .unwrap();
        assert_sidecars_empty(&dir);
    }
