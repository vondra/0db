...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/ss`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// South Sudan bbox: [minLat, minLon, maxLat, maxLon]
const SS_BBOX: [number, number, number, number] = [3.5, 24.0, 12.2, 36.0]
...
/**
 * Returns true if the point is clearly outside South Sudan.
 */
function isExcluded(lat: number, lon: number): boolean {
  if (lat > 12.2) return true          // Sudan N
  if (lat < 3.5) return true           // DRC / Uganda / Kenya S
  if (lon < 24.0) return true          // CAR W
  if (lon > 36.0) return true          // Ethiopia / Kenya E
  return false
}
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, SS_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'SS plant').toString(),
...
async function main() {
  console.log(`=== SS Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in SS: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, SS_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  SS-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM SS (${best.fuel})` }
          matched++
        }
      }
    } catch {}
