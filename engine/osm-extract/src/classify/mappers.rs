...
/// `noise_compute::emission::leisure`). `sport=*` wins; otherwise the
/// `leisure=*`/`amenity=*` kind selects a default (playground, pool, pitch,
/// outdoor seating, stadium). Always returns a class (PITCH is the fallback).
pub fn leisure_sport_class(tags: &Tags) -> u8 {
    use noise_compute::emission::leisure as lz;
    if let Some(sport) = tags.get("sport") {
        // Multi-value `sport=tennis;padel` → take the loudest by trying each.
        if let Some(c) = sport
            .split(';')
    ...
    {
        return lz::OUTDOOR_SEATING;
    }
    match tags.get("leisure").map(|s| s.as_str()) {
        Some("playground" | "dog_park") => lz::PLAYGROUND,
        Some("swimming_pool" | "swimming_area" | "water_park") => lz::POOL,
        Some("stadium") => lz::STADIUM,
        Some("outdoor_seating") => lz::OUTDOOR_SEATING,
        _ => lz::PITCH,
    }
}

/// Parse OSM `capacity`/`seats` to a person count for leisure crowd scaling.
/// First numeric token only; `None` for non-numeric or absent.
pub fn parse_capacity(tags: &Tags) -> Option<u32> {
    tags.get("seats")
        .or_else(|| tags.get("capacity"))
        .and_then(|s| {
            let n: String = s
                .trim()
                .chars()
...
/// in `spill.rs` so they can never collide. Defined in `noise-compute`
/// (single source of truth) because `normalize_road` resolves the sentinel
/// to its effective derestricted speed.
pub use noise_compute::normalize::SPEED_LIMIT_DERESTRICTED;
...
mod leisure_tests {
    ...
    fn leisure_sport_class_resolution() {
        assert_eq!(leisure_sport_class(&tags(&[("sport", "padel")])), lz::PADEL);
        assert_eq!(
    ...
    #[test]
    fn capacity_parse() {
        assert_eq!(parse_capacity(&tags(&[("seats", "120")])), Some(120));
        assert_eq!(
            parse_capacity(&tags(&[("capacity", "80 people")])),
            Some(80)
        );
        assert_eq!(parse_capacity(&tags(&[("capacity", "lots")])), None);
        assert_eq!(parse_capacity(&tags(&[])), None);
    }
