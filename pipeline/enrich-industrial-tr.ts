...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/tr`)
...
const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Greece/Bulgaria W — west of 26.5 above lat 40.5
  [40.5, 25.6, 42.2, 26.5],
  // Georgia NE — east of 42.5 above lat 41
  [41.0, 42.5, 42.2, 44.8],
  // Armenia NE — east of 43.5 above lat 39
  [39.0, 43.5, 42.2, 44.8],
  // Iran E — east of 44.0 below lat 39.8
  [35.8, 44.0, 39.8, 44.8],
  // Iraq SE — east of 42 below lat 37.5 (narrow SE corner)
  [35.8, 42.0, 37.5, 44.8],
  // Syria S — below lat 36.2 east of 36
  [35.8, 36.0, 36.2, 44.8],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, TR_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'TR plant').toString(),
...
async function main() {
  console.log(`=== TR Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in TR: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, TR_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  TR-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM TR (${best.fuel})` }
          matched++
        }
      }
    } catch {}
