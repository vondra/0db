 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-industrial-st.ts
 */
...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/st`)

// São Tomé and Príncipe bbox: [minLat, minLon, maxLat, maxLon]
const ST_BBOX: [number, number, number, number] = [0.0, 6.4, 1.7, 7.5]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, ST_BBOX)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'ST plant').toString(),
...
async function main() {
  console.log(`=== ST Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)
  console.log(`  Note: GEM reports 0 operating plants for São Tomé and Príncipe.\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in ST: ${plants.length}`)
  ...
  const MY_DATASET_ID = DATASETS_BY_KEY.get('st-industrial')!.id
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, ST_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  ST-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
