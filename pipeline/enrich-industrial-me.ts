 ...
 *
 * Usage:
 *   DATA_YEAR=2025 npx tsx pipeline/enrich-industrial-me.ts
 */
...
const CACHE_DIR = resolve(import.meta.dirname, `../data/enrichment/${YEAR}/me`)
const NACE_LOOKUP_PATH = resolve(import.meta.dirname, `../data/prepared/nace-lookup.json`)

// Montenegro bbox [minLat, minLon, maxLat, maxLon]
const ME_BBOX: [number, number, number, number] = [41.85, 18.4, 43.6, 20.4]

const EXCLUDE_ZONES: Array<[number, number, number, number]> = [
  // Serbia N — above lat 43.45
  [43.45, 18.4, 43.6, 20.4],
  // Bosnia W — west of 18.6 above lat 42.7
  [42.7, 18.4, 43.6, 18.6],
  // Albania S — below lat 42.0
  [41.85, 18.4, 42.0, 20.4],
  // Kosovo E — east of 20.3 above lat 42.3
  [42.3, 20.3, 43.6, 20.4],
]
...
function loadGemPlants(): IndSite[] {
  ...
  for (const f of fc.features || []) {
    ...
    if (!inBbox(lat, lon, ME_BBOX) || inExcluded(lat, lon)) continue
    const p = f.properties || {}
    const status = (p.Status || '').toString().toLowerCase()
    if (!status.includes('operating')) continue
    out.push({
      lat, lon,
      name: (p.Plant___Project_name || 'ME plant').toString(),
...
async function main() {
  console.log(`=== ME Industrial Enrichment — GEM Global Integrated Power (${YEAR}) ===\n`)

  const plants = loadGemPlants()
  const fuelCounts: Record<string, number> = {}
  for (const p of plants) fuelCounts[p.fuel] = (fuelCounts[p.fuel] || 0) + 1
  console.log(`  GEM operating plants in ME: ${plants.length}`)
  ...
  for (const hex of allHexes) {
    try {
      ...
      if (inBbox(lat, lon, ME_BBOX) && existsSync(resolve(H3R4_DIR, hex, 'industrial.arrow'))) hexDirs.push(hex)
    } catch {}
  }
  console.log(`  ME-bbox hexes with industrial.arrow: ${hexDirs.length}\n`)
    ...
    try {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (best) {
          ...
          lookup[id] = { nace: naceCode, name: best.name, source: `GEM ME (${best.fuel})` }
          matched++
        }
      }
    } catch {}
