...
use crate::types::{
    BaselineTrace, ForestRun, GroundTrace, LdenVariants, PathProfileTrace, PerPeriod,
    PropagationVariants, ScreeningObstacleTrace, ScreeningTrace, TerrainTrace, VegetationTrace,
    NUM_BANDS,
};
...
use crate::types::{
    CruiseBucketBreakdown, EmissionTrace, LayerKind, PointSource, RailSegment, RoadSegment,
    SegmentTrace,
};
...
// SegmentTrace covers three aircraft sub-types via `aircraft_subtype`:
// 1 = ground path (`GroundPathView`), 2 = airborne sub-segment (one
// Doc 29 SEL call), 3 = cruise R8 hex (per-r8 aggregate carrying a
// per-bucket breakdown). Builders below pull values the engine
// already holds; no re-emission or re-propagation happens here.
...
pub fn build_aircraft_ground_path_trace(
    ...
    let (start, end) = match (polyline.first(), polyline.last()) {
        (Some(s), Some(e)) => (*s, *e),
        _ => ((0.0, 0.0), (0.0, 0.0)),
    };
    let typecode_str = std::str::from_utf8(aircraft_type)
        .unwrap_or("")
        .trim_end_matches('\0')
        .to_string();
    let _ = flight_id;
...
pub fn build_aircraft_airborne_subsegment_trace(
    ...
    let typecode_str = std::str::from_utf8(inputs.aircraft_type)
        .unwrap_or("")
        .trim_end_matches('\0')
        .to_string();
    let _ = inputs.flight_id;
    SegmentTrace {
        kind: LayerKind::Aircraft,
        osm_id: None,
        segment_idx: 0,
