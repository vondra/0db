...
async function enrichHexes(allStopCounts: StopTrainCount[], retractSafe: boolean): Promise<void> {
  ...
  // so the raw stop list may contain foreign stations (VIA/Amtrak joint runs cross
  // into the US) — joining those would stamp a neighbour's track under this feed's
  ...
  allStopCounts = allStopCounts.filter((sc) => inCa(sc.lat, sc.lon))
  ...
  // Scan ALL Canadian hexes (not just ones with stops) so class defaults reach
  // every track — a tram that no longer inherits a commuter count gets its own value.
  const hexDirs = iterateCountryHexes(H3R4_DIR, CA_BBOX, 'railways.arrow')
  console.log(`  CA hexes with railways.arrow: ${hexDirs.length}`)
  ...
  for (let hi = 0; hi < hexDirs.length; hi++) {
    ...
    const r = await writeRailTrains(
      ...
      (row) => {
        // Family gate: heavy rail (rail_type 0) → commuter/VIA rail stops; tram/light_rail
        // (rail_type 1/2) → streetcar/LRT/metro stops. Cross-family matches can't happen.
      ...
      retractSafe ? {
        ...
        when: (row) => {
          // Country-bleed disown (#26C): ANY owned row physically wholly outside CA (start+mid+end — genuine border-straddlers stay ours; shared R9 predicate) is
          ...
          if (segmentWhollyOutside(inCa, row.midLat, row.midLon, row.startLat, row.startLon, row.endLat, row.endLon)) return true
          if (!wasOldFallbackStamp(row)) return false
          const grid = row.railType === 0 ? railGrid : (row.railType === 1 || row.railType === 2) ? tramGrid : null
          return !grid || nearestGridStop(grid, row) === null
        },
      } : undefined,
      inCa, // #31.7 central country gate — see writeRailTrains
    )
    totalRails += r.rows
...
async function main() {
  ...
  // snapshot — all eight feeds loaded non-empty THIS run (or a v2 cache that
